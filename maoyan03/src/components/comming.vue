    ...
    <loading v-if="!isReady"></loading>
      <div class="hopeful">
        ...
        <div class="img-list" ref="imglist">
          <div class="hopeful-list" v-for="(item, index) in hopeList" :key="index">
            <div class="hope-img">
      ...
      <div class="comming">
        <movieList v-for="(c_item,index) in comList" :key="index" :mitem="c_item"></movieList>
      </div>
  </div>
</template>
...
export default {
  ...
  created() {
    function getComingList(){
      return axios.get("http://www.softeem.xin/maoyanApi/ajax/comingList?ci=57&token=&limit=10")
    }
    
    function getHopefulList(){
      return axios.get("http://www.softeem.xin/maoyanApi/ajax/mostExpected?ci=57&limit=10&offset=0&token=")
    }
    
    axios.all([getComingList(), getHopefulList()]).then(
      axios.spread((comRes, hopeRes) => {
        this.comList = comRes.data.coming;
        this.hopeList = hopeRes.data.coming;
        this.isReady = true;
      })
    );
